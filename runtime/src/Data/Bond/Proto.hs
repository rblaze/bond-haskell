...
class (Default a, WireType a) => BondSerializable a where
    -- | Read field value.
    bondGet :: BondProto t => BondGet t a
    -- | Put field into stream.
    bondPut :: BondProto t => a -> BondPut t

class (Default a, BondSerializable a) => BondStruct a where
    -- | Read struct from untagged stream
    bondStructGetUntagged :: BondProto t => BondGet t a
    bondStructGetBase :: BondProto t => a -> BondGet t a
    bondStructGetField :: BondProto t => Ordinal -> a -> BondGet t a
    -- | Put struct
    bondStructPut :: BondProto t => a -> BondPut t
